...
# ===========================
  ...
  data2010 <- read_csv("data/csv/london/raw/MPS Ward Level Crime 2010-2020-03.csv") %>%
    ...
    # select less columns
    data2010 = select(data2010, WardCode, MajorCategory, Year2011, Year2012, Year2013, Year2014, Year2015,
      Year2016, Year2017, Year2018, Year2019, Year2020_A)
  
    # Row Summary (We need to do this because of MinorCategorie are different
    # in 2020 and 2001.)
    data2010 = aggregate(YearSum ~ WardCode + MajorCategory,
      data = data2010, FUN = sum, na.rm = TRUE)
...
# ===========================
  ...
  data2020 <- read_csv("data/csv/london/raw/MPS Ward Level Crime (most recent 24 months).csv") %>%
    ...
    # select less columns
    data2020 = select(data2020, WardCode, MajorCategory, Year2020_B, Year2021, Year2022)
  
    # Row Summary (We need to do this because of MinorCategorie are different
    # in 2020 and 2001.)
    data2020 = aggregate(YearSum ~ WardCode + MajorCategory,
...
# ============================ 
  ...
  OVERLAPPING_CATEGORIES = intersect(data2010$MajorCategory, data2020$MajorCategory)
  
  data2010 = filter( data2010, MajorCategory %in% OVERLAPPING_CATEGORIES) 
  data2020 = filter( data2020, MajorCategory %in% OVERLAPPING_CATEGORIES) 
